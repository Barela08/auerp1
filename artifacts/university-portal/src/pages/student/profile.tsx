...
import { AlertCircle, User, GraduationCap, Phone, Mail, MapPin, Printer, ShieldCheck } from "lucide-react";
...
export function ProfilePage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-8">
        ...
        <CardContent className="p-6 pt-0 -mt-10">
          <div className="flex items-end gap-5 flex-wrap">
            ...
            <div className="pb-2">
              ...
              <p className="text-gray-500 text-sm">{(student as any).universityRegNo || student.enrollmentNo}</p>
            </div>
            <div className="ml-auto pb-2 flex gap-2 flex-wrap">
              {(student as any).bloodGroup && (
                <Badge className="bg-red-100 text-red-700 hover:bg-red-100">
                  Blood: {(student as any).bloodGroup}
                </Badge>
              )}
              <Badge className="bg-primary/10 text-primary hover:bg-primary/10">
                Sem {student.semester}
              </Badge>
              {(student as any).attendancePct && (
                <Badge className={`hover:bg-green-100 ${(student as any).attendancePct >= 75 ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"}`}>
                  {(student as any).attendancePct}% Attendance
                </Badge>
              )}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="p-4">
            ...
            <InfoRow label="University Regn. No." value={(student as any).universityRegNo} />
            ...
            <InfoRow label="SGPA" value={(student as any).sgpa ? (student as any).sgpa.toFixed(2) : null} />
            {(student as any).attendancePct && (
              <InfoRow label="Attendance" value={`${(student as any).attendancePct}%`} />
            )}
          </CardContent>
        </Card>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="p-4">
            ...
            <InfoRow label="Blood Group" value={(student as any).bloodGroup} />
            <InfoRow label="Category" value={(student as any).category} />
      ...
      {/* ID Information */}
      {((student as any).aadhaarNo || (student as any).universityRegNo) && (
        <Card className="shadow-sm">
          <CardHeader className="border-b pb-4">
            <CardTitle className="text-base flex items-center gap-2">
          ...
          <CardContent className="p-4 grid grid-cols-1 md:grid-cols-2 gap-x-8">
            <InfoRow label="University Regn. No." value={(student as any).universityRegNo} />
            <InfoRow label="Aadhaar No." value={(student as any).aadhaarNo} />
            <InfoRow label="Category" value={(student as any).category} />
            <InfoRow label="Blood Group" value={(student as any).bloodGroup} />
          </CardContent>
        </Card>
